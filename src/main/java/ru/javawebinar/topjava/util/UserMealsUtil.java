...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal m : meals) {
            ...
            sumCaloriesByDate.put(date, sumCaloriesByDate.getOrDefault(m.getDateTime().toLocalDate(), 0) + m.getCalories());
        }
        ...
        for (UserMeal m : meals) {
            if (TimeUtil.isBetweenHalfOpen(m.getDateTime().toLocalTime(), startTime, endTime)) {
                boolean excess = sumCaloriesByDate.get(m.getDateTime().toLocalDate()) > caloriesPerDay;
                mealWithExcesses.add(new UserMealWithExcess(m.getDateTime(), m.getDescription(), m.getCalories(), excess));
            }
        }
